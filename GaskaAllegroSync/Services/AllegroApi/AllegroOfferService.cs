        ...
        public async Task SyncAllegroOffersDetails(CancellationToken ct = default)
        {
            try
            {
                ...
                foreach (var offer in allOffers)
                {
                    ...
                    {
                        Log.Error(ex, "Exception while fetching details for offer ID {OfferId}", offer.Id);
                    }
                }
                ...
                Log.Information("Fetched and saved {Count} offer details from Allegro.", offersDetails.Count);
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Fatal error while fetching and saving offers.");
