...
namespace GaskaAllegroProductsSync.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task UpsertProducts(List<ApiProducts> apiProducts, HashSet<int> fetchedProductIds, CancellationToken ct)
        {
            ...
            // 2. Load existing products in one query
            var productIds = apiProducts.Select(p => p.Id).ToList();
            var existingProducts = (await conn.QueryAsync<Product>(
                "SELECT * FROM Products WHERE Id IN @Ids", new { Ids = productIds }, tran
            )).ToDictionary(p => p.Id);
        ...
        public async Task<List<Product>> GetProductsToUpload(CancellationToken ct)
        {
            ...
            while (true)
            {
                ...
                // 3️⃣ Map child collections to each product
                foreach (var p in products)
                {
                    p.Images = images
